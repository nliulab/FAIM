...
def plot_distribution(df, s = 4):
        ...
        for i, x in enumerate(df.columns[:-2]):
            ...
            sns.histplot(data=df, x=x, hue="sen_var_exclusion",  bins=50, ax=ax, legend=False)#layout=(1, num_metrics), figsize=(4, 4), color="#595959",
            ax.set_title(x)
            ax.set_xlabel("")
            ax.set_ylabel("Count" if i == 0 else "")
        
        plt.legend(loc="center left", title="", labels=labels[::-1], ncol=1,  bbox_to_anchor=(1.04, 0.5), borderaxespad=0)
    ...
    for k, s in enumerate(shapes_candidates):
        for i in range(num_metrics):
            ...
            if k == 0:
                ...
                min_annotation = {'x': 0,'y': min_metric, 'text': f'Model ID {best_id}<br> Rank No.1', 'showarrow': True, 'arrowhead':6, 'xanchor':'left','yanchor':'bottom', 'xref': 'x', 'yref': 'y', 'font': {'size': anno_size}, 'ax':-10, 'ay':-10, 'xshift':0,'yshift':0}
                fig.add_annotation(min_annotation,  col=i+1, row=1)
            if meduim_id in ids:
                medium_annotation = {'x': math.log10(int(num_models/2)+1),'y': meduim_metric+jitter_control[meduim_id], 'text': f'Model ID {meduim_id}<br> Rank No.{int(num_models/2)}', 'showarrow': True, 'arrowhead':6, 'xanchor':'left','yanchor':'bottom', 'xref': 'x', 'yref': 'y', 'font': {'size': anno_size}, 'ax':-10, 'ay':-10, 'xshift':0,'yshift':0}
                fig.add_annotation(medium_annotation,  col=i+1, row=1)
            if worst_id in ids:
                max_annotation = {'x': math.log10(num_models+1),'y': max_metric+jitter_control[worst_id], 'text': f'Model ID {worst_id}<br> Rank No.{num_models}', 'showarrow': True, 'arrowhead':6, 'xanchor':'left','yanchor':'bottom', 'xref': 'x', 'yref': 'y', 'font': {'size': anno_size}, 'ax':-10, 'ay':-10, 'xshift':0,'yshift':0, "align":'left'}
                fig.add_annotation(max_annotation,  col=i+1, row=1)
    
    colorbar_trace  = go.Scatter(x=[None],y=[None],mode='markers', hoverinfo='none', marker=dict(
                                 colorscale= [rgb01_hex(np.array((243, 244, 245))/255), "steelblue"], #"magma", 
                                 showscale=True,
                                 cmin=0,
                                 cmax=2,
                                 colorbar=dict(title = None, thickness=10, tickvals=[0, 2], ticktext=['Low', 'High'], outlinewidth=0, orientation='v', x=1, y=0.5)),
                            )
...
def plot_radar(df, thresh_show, title, **kwargs):
    ...
    print(f"The best model is No.{best_id} with metrics on validation set:\n {df.loc[best_id, :]}")
...
def plot_bar(shap_values, feature_names, original_feature_names, coef=None, title=None, **kwargs):
    ...
    if shap_values is not None:
        ...
        grouped_df = pd.DataFrame({"values":shap_values}, index=feature_names).groupby(by = get_prefix, axis=0)
        ...
        df = pd.concat([df, pd.DataFrame({"color": ["grey" if i < 0 else "steelblue" for i in df.Value], "order": np.abs(df.Value)})], axis=1)
        
    elif coef is not None:
        df = pd.DataFrame({"Var":coef.index, "Value":coef.values, "color": ["grey" if i < 0 else "steelblue" for i in coef.values], "order": np.abs(coef.values)})
    else:
...
# def plot_fairlite(y_true, y_pred, sens_name, sens_var_ref):
#     obj = FairEvaluator(y_obs=y_true, sens_var=sens_name, sens_var_ref = sens_var_ref, y_pred = y_pred, y_pred_threshold=None)
