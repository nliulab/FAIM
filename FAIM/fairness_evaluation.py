...
class FAIMEvaluator():
    def __init__(self, y_true, y_pred, y_pred_bin, sen_var, fair_only=False, cla_metrics=["auc"], weighted=False, weights=None, bases=my_fairness_bases, bound=my_bases_bound):  
    ...
    def _fairmetrics_generation(self):
        ...
        if self.weighted:
            fairmetrics["BER Equality"] = self.weights["tpr"] * diff_["tpr"] + self.weights["tnr"] * diff_["tnr"]
    ...
    def _clametric_generation(self):
        ...
        if "auc" in self.cla_metrics:
            ...
            clametrics["auc_low"], clametrics["auc"], clametrics["auc_high"]= get_ci_auc(self.y_true, pred, alpha=0.05, type="auc")
