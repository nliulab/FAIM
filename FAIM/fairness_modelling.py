...
class FAIMGenerator(FairBase):
    ...
    def fairness_compute(self, dat_expl, optim_base_obj, coefs, weighted = True, weights={"tnr": 0.5, "tpr": 0.5}, **kwargs):
        ...
        for i in range(coefs.shape[0]):
            ...
            fe = FAIMEvaluator(y_true=y_expl, y_pred=prob_expl, y_pred_bin=pred_expl, sen_var=sen_var, fair_only=True, weighted=weighted, weights=weights)
...
`
        ...
        fe = FAIMEvaluator(y_true=np.array(y_test), y_pred_bin=pred_test, y_pred=prob_test,sen_var=sen_var, weighted=self.weighted, weights=self.weights)
        fairmetrics = fe.fairmetrics
