...
class FAIRegGenerator(FairBase):
    def __init__(self, dat_train, selected_vars, selected_vars_cat, y_name, sen_name, sen_var_ref, output_dir, criterion="loss", epsilon=0.05, m=800, n_final=350, without_sen=False, pre=False, pre_method="Reweigh", post=False, post_method="equalizedodds"):
        """Initialize the class of FAIReg
        ...
        Args:
            ...
            pre (bool, optional): whether to use pre-process bias mitigation methods before FAIReg. Defaults to False.
            pre_method (str, optional): specific pre-process method. Defaults to "Reweigh".
            post (bool, optional): whether to use post-process bias mitigation methods after FAIReg. Defaults to False. 
            post_method (str, optional): specific post-process method. Defaults to "EqOdds".
        """
        
        super().__init__(dat_train, selected_vars, selected_vars_cat, y_name, sen_name, sen_var_ref, without_sen)
    ...
    def pre_mitigate(self):
        ...
        """
        rw_model, rw_results, instance_weights = self.model(method_type="pre", method=self.pre_method)
    ...
    def optimal_model(self, selected_vars, selected_vars_cat):
        ...
        """
        x = self.dat_train.drop(columns=[c for c in self.dat_train.columns if c == self.y_name or c not in selected_vars]) 
        ...
        model_object = model.models(
            ...
            sample_w = self.rw_weights if self.pre and self.pre_method == "rw" else None #[np.mean(dat_train[y_name]) if i == 0 else 1-np.mean(dat_train[y_name]) for i in dat_train[y_name]]
        ) 
        return model_object
    ...
    def nearly_optimal_model(self, optim_base_obj, m=200, n_final=50, epsilon=None):
        ...
        optim_base_obj.draw_models(
            u1=u1, u2=u2, m=self.m, n_final=self.n_final, random_state=1234, epsilon=epsilon, criterion=self.criterion)
        coefs = pd.read_csv(os.path.join(self.output_dir, "models_near_optim.csv"), index_col=0)
        return coefs, optim_base_obj.models_plot
        
        
    def fairness_compute(self, dat_expl, optim_base_obj, coefs, weighted = True, weights={"tnr": 0.5, "tpr": 0.5}, **kwargs):
        ...
        for i in range(coefs.shape[0]):
            ...
            # prob_expl = 1/(np.exp(-(x_with_constant @ coef).astype("float")) + 1)
            thresh = find_optimal_cutoff(y_expl, prob_expl)[0]
            # thresh_list.append(thresh)
            pred_expl = prob_expl > thresh
            
            # print(inspect.getsource(FairEvaluator.__init__))
            fe = FAIRegEvaluator(y_true=y_expl, y_pred=prob_expl, y_pred_bin=pred_expl, sen_var=sen_var, weighted=weighted, weights=weights)
    ...
    def compare(self, dat_expl, optim_base_results, selected_vars, selected_vars_cat):
        ...
        """
        x_with_constant_ori, sen_var, y_expl = self.data_process(dat_expl, selected_vars=self.vars, selected_vars_cat=self.vars_cat)
        x_with_constant_base, sen_var, y_expl = self.data_process(dat_expl, selected_vars=selected_vars, selected_vars_cat=selected_vars_cat)
    ...
    def FAIReg_model(self, dat_expl):
        """FAIReg: Generate the nearly optimal models and compute the fairness metrics of the nearly optimal models
        ...
        if self.without_sen == "auto":
            ...
            pbar =  tqdm(sen_senarios, desc="Generating nearly optimal models", postfix="*Start*")
            for x in pbar:
                ...
                if self.compare(dat_expl, optim_base_results, selected_vars, selected_vars_cat):
                    ...
                    fairmetrics_df, qc_df = self.fairness_compute(dat_expl, optim_base_obj, coefs, selected_vars=selected_vars, selected_vars_cat=selected_vars_cat)
                    ...
                    self.fairmetrics_df = pd.concat([self.fairmetrics_df, fairmetrics_df])
    ...
    def describe(self, selected_metrics=None):
        ...
        ids_after_qc = np.arange(qc_df.shape[0])[(np.sum(qc_df, 1) == qc_df.shape[1]).tolist()]
        ...
        for m in min_ones.index:
            ...
            print(f"the model with minimal {m}: No.{id} -- {fairmetrics_df.loc[id, m]:.3f}, with {sen_var_exclusion[id]} excluded from regression")
        
        plot_df = self.fairmetrics_df.loc[ids_after_qc, :]
        fig = plot_distribution(plot_df)
        
        if len(ids_after_qc) < fairmetrics_df.shape[0]:
            return fairmetrics_df.iloc[ids_after_qc, :], [sen_var_exclusion[i] for i in ids_after_qc], fig
        
        return fig
    
    def transmit(self, targeted_metrics = ["Average Accuracy", "Statistical Parity", "Equalized Odds"], thresh_show=0.3, **kwargs):
        ...
        """
        FAIRegarea_list = []
        ids = np.sum(self.fairmetrics_df[targeted_metrics] < thresh_show, axis=1) == len(targeted_metrics)
        if len(ids) == 0:
        ...
        for i, id in enumerate(df.index): 
            values = df.loc[id, :]
            FAIRegarea_list.append(fairarea(values))
            
        self.best_id = df.index[np.argmin(FAIRegarea_list)]
        self.best_sen_exclusion = sen_var_exclusion.iloc[np.argmin(FAIRegarea_list)]
        
        id_senario = [index for index, item in enumerate(list(self.coefs.keys())) if item==self.best_sen_exclusion][0]
        ...
        x_with_constant = self.data_process(dat_uncertainty, selected_vars=selected_vars, selected_vars_cat=selected_vars_cat)[0].values
        
        prob_train, _, _ = self.test(dat_uncertainty)
        best_se = None
        fisher_information = x_with_constant.T @ np.diag(prob_train * (1-prob_train)) @ x_with_constant
        # fisher_information = arma.mat(x_with_constant.T) * arma.mat(np.diag(prob_train * (1-prob_train))) * arma.mat(x_with_constant)
        print("multiplication successed!")
        cov = np.linalg.pinv(fisher_information)
        # cov = np.array(arma.pinv(fisher_information))
        best_se = [np.sqrt(cov[i, i]) for i in range(cov.shape[0])]
        
        # self.best_thresh = self.thresh_list[self.best_id]
        
        return self.best_coef, self.best_sen_exclusion, best_se, self.best_optim_base_obj.model_optim, p, fair_idx_df
    ...
    def test(self, dat_test, model=None, params=None, thresh=None):
        ...
        +--------------+------------+----------------------------------------+
        | None         | None       |Test the best model produced by FAIReg. |
        +--------------+------------+----------------------------------------+
...
`
        ...
        selected_vars = [i for i in self.vars if i not in excluded_vars]
        selected_vars_cat = [i for i in self.vars_cat if i not in excluded_vars]
        x_with_constant, sen_var, y_test = self.data_process(dat_test, selected_vars=selected_vars, selected_vars_cat=selected_vars_cat)  
        
        if model is None:
            prob_test = self.best_optim_base_obj.model_optim.model.predict(params=self.best_coef, exog=x_with_constant)
        else:
        ...
        fe = FAIRegEvaluator(y_true=y_test, y_pred_bin=pred_test, y_pred=prob_test,sen_var=sen_var, weighted=self.weighted, weights=self.weights)
        fairmetrics = fe.fairmetrics
        
        return prob_test, fairmetrics, fe.fairsummary
    ...
    def explain(self, method="best"):
        ...
        bg_data = self.data_process(
                ...
                selected_vars_cat=selected_vars_cat)[0].sample(n=1000, random_state=42)
        ex_data = self.data_process(
            self.dat_expl, 
            selected_vars=selected_vars, 
            selected_vars_cat=selected_vars_cat)[0].sample(n=200, random_state=42)
            
        e = shap.KernelExplainer(f, bg_data)
        shap_values_train = e.shap_values(ex_data)
        shap_values_train_1 = shap_values_train[1].squeeze()
        pd.DataFrame(shap_values_train_1).to_csv(os.path.join(output_dir, f"{method}.csv"))
    ...
    def compare_explain(self, overide=True):
        ...
        if (not os.path.exists(os.path.join(output_dir, "best.csv"))) or overide:
            ...
            shap_best = pd.read_csv(os.path.join(output_dir, "best.csv"), index_col=0).values.reshape(-1)
