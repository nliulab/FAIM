...
class FairBase:
    ...
    def test(self, dat_test, model=None, params=None, thresh=None, **kwargs):
        ...
        fe = FAIRegEvaluator(y_true=y_test, y_pred=prob_test,y_pred_bin=pred_test, sen_var=sen_var, weighted=self.weighted, weights=self.weights)
        fairmetrics = fe.fairmetrics
        fairsummary = fe.fairsummary
        
        return pred_test if prob_test is None else prob_test, fairmetrics, fairsummary, thresh